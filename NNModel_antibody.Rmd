...
```{r}
igg_training = training_data$plasma_antibody_levels$wide
dim(igg_training)
sum(is.na(igg_training))
library(impute)
library(purrr)
library(dplyr)
normalized_imputed = igg_training[rowMeans(is.na(igg_training)) < 1, ] %>%
    as.matrix() %>%
    impute.knn() %>%
    .$data%>%as.data.frame()
...
selected_columns <- c("biological_sex", "infancy_vac", "age_at_event", "IgG_PT_D14", mean_columns)
...
formula <- as.formula(
  paste("IgG_PT_D14 ~ biological_sex + infancy_vac + age_at_event +", paste(mean_columns, collapse = " + "))
)
...
```
```{r}
test_data = testing_data$plasma_antibody_levels$wide
dim(test_data)
sum(is.na(test_data))
...
selected_columns <- c("subject_id","biological_sex", "infancy_vac", "age_at_event","IgG_PT_D0", mean_columns)
...
original_min <- min(df_pivot_training)  
original_max <- max(df_pivot_training)  
...
test_data$fold_change <- test_data$results / df_pivot2$IgG_PT_D0
...
print(ranked_data)
ranked_data_IGG = ranked_data
save(ranked_data_IGG, file = "~/CMI_IGG.Rdata")
```