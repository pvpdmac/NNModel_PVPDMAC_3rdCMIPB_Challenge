...
```{r}
cf_training = training_data$pbmc_cell_frequency$wide
dim(cf_training)
sum(is.na(cf_training))
library(impute)
library(purrr)
library(dplyr)
normalized_imputed = cf_training[rowMeans(is.na(cf_training)) < 1, ] %>%
    as.matrix() %>%
    impute.knn() %>%
    .$data%>%as.data.frame()
...
safe_column_names <- function(col_names) {
  ...
  col_names <- gsub("\\)", "", col_names)  # Remove ")"
  col_names <- gsub("\\+", "_plus_", col_names)  # Replace "+" with "_plus_"
  return(col_names)
}
df_pivot1 = df_pivot
  ...
  lapply(function(feature) {
   feature_d0 <- paste0(feature, "_D0")
    ...
    if (all(c(feature_d0, feature_d15, feature_d30) %in% colnames(df_pivot1))) {
      ...
      # Return a data frame with mean column
...
selected_columns <- c("biological_sex", "infancy_vac", "age_at_event", "Monocytes_D1", mean_columns)
...
formula <- as.formula(
  paste("Monocytes_D1 ~ biological_sex + infancy_vac + age_at_event +", paste(mean_columns, collapse = " + "))
)
...
```
```{r}
test_data = testing_data$pbmc_cell_frequency$wide
dim(test_data)
sum(is.na(test_data))
...
selected_columns <- c("subject_id","biological_sex", "infancy_vac", "age_at_event","Monocytes_D0", mean_columns)
...
nn_predictions_clipped <- pmax(pmin(predicted_values, 1), 0)

original_min <- min(df_pivot_training$Monocytes_D1)  
original_max <- max(df_pivot_training$Monocytes_D1)  
...
```
```{r}
original_subject_id <- df_pivot2$subject_id

test_data$fold_change <- test_data$results / df_pivot2$Monocytes_D0
...
print(ranked_data)
ranked_data_CF = ranked_data
save(ranked_data_CF, file = "~/CMI_CF.Rdata")
```
